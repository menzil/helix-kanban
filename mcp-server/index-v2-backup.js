...
function parseProjectList(output) {
  ...
  for (const line of lines) {
    if (
      line.includes("全局项目") ||
      line.includes("Global projects") ||
      line.includes("本地项目") ||
      line.includes("Local projects") ||
      line.includes("========")
    ) {
      continue;
    }

    const match = line.match(/^\s*\d+\.\s*\[([GL])\]\s*(.+)$/);
    if (match) {
      projects.push({
...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
  ...
  if (!match) {
    throw new Error("Invalid kanban URI format. Expected: kanban:///project-name");
  }
...
async function main() {
  ...
  console.error("Helix Kanban MCP server v2.0.0 running on stdio");
}
